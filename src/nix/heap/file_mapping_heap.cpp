#if 0
#include <nix/heap/file_mapping_heap.h>
#include <nix/memory.h>
#include <nix/io/memory.h>
#include <nix/serialize/s11n.h>
#include <nix/utility/unused.h>
...
namespace nix
{
	...
	struct file_mapping_heap_imp
	{
		...
		{
			XR_PRE_CONDITION(pin_map.empty());
			XR_PRE_CONDITION(no_pinned_memory());
			unload_();
		}
		...
		{
			XR_PRE_CONDITION(track_pin_count(h) == 0);
		...
		long_offset_t get_handle(const byte* ap) const
		{
			...
			XR_PRE_CONDITION (itr != view_map.right.begin());	// fail if found!
			--itr;
			XR_PRE_CONDITION (itr->info > 0 );	//view counter != 0
			byte* p_start = itr->first;
			byte* p_end = itr->first + itr->second.size();
			unused(p_end, p_start);
			XR_PRE_CONDITION (p >= p_start && p < p_end);
			...
			if (lower != outer_free_().end())
			{
				if (lower->begin() < ha.end())
				{
					...
					if (h_.end() >= ha.end())
					{
						...
						outer_free_().insert(handle(ha.end(), h_.end()));
					}
				}
			}
		...
		int unpin_(byte* p, bool track)
		{
			...
			if(track)
			{
				...
				XR_PRE_CONDITION(pos != pin_map.end());
				if (--pos->second == 0)
				{
					pin_map.erase(pos);
					return 0;
		...
		void load_() {
			...
			if (arinfo.size() < tail_size)
				XR_THROW(ext_heap_format_exception);
	...
	{
		std::lock_guard<std::mutex> lock(m_imp->mutex);
