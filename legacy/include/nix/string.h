...
#ifndef XR_STRING_H__
#define XR_STRING_H__
...
#include <xirang2/string/shared_data.h>
...
namespace xirang2
{
	...
	public:
			...
			: m_beg(first), m_end(last)
		{
			...
			XR_PRE_CONDITION(first == last || first != 0);
		}
		...
		reference operator[] (size_type index) const{
			XR_PRE_CONDITION(index >= 0 && index < size());
			return m_beg[index];
		}
	...
	private:
		...
		{
			XR_PRE_CONDITION(n > 0);
            
            auto p_ = hp.malloc(
					sizeof(data_type) +  sizeof(CharT) * n
				, sizeof(std::size_t), 0);
            data_type* p = static_cast<data_type* >(p_);
	...
	public:
		...
		basic_string_builder(const_pointer rhs, size_type pos = 0, size_type n = npos)
			...
			, m_data(0)
		{
			XR_PRE_CONDITION(rhs != 0 && pos <= traits_type::length(rhs));
			n = n == npos ? traits_type::length(rhs + pos) : n;

			XR_PRE_CONDITION(pos + n <= traits_type::length(rhs));
		...
		{
			XR_PRE_CONDITION(p >= begin() && p < end());
			std::copy(p + 1, end(), p);
			m_data[--m_size] = CharT();
			return p;
		}
		...
		{
			XR_PRE_CONDITION(r.begin() <= r.end());
			XR_PRE_CONDITION(r.begin() >= begin() && r.begin() <= end());
			XR_PRE_CONDITION(r.end() >= begin() && r.end() <= end());
			if (!empty())
			{
		...
		{
			XR_PRE_CONDITION(m_data > 0 &&  n < m_size);
			return m_data[n];
		}
