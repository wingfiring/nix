...
namespace xirang2 { namespace io{
	...
	const long_size_t unknow_size = long_size_t(-1);

	//reader
	struct XR_INTERFACE reader
	{
	...
	//forward
	struct XR_INTERFACE forward : sequence
	{
		/// \return current seek position.
		virtual long_size_t offset() const = 0;
		/// \return archive length, may return unknow_size.
	...
	//random
	struct XR_INTERFACE random : forward
	{
		/// \return current seek position.
		virtual long_size_t offset() const = 0;
		/// \return archive length, may return unknow_size.
	...
	//write_view
	struct XR_INTERFACE write_view
	{
	...
	XR_API extern range<reader::iterator> block_read(reader& rd, const range<reader::iterator>& buf);
	...
	/// \return rest part of given buf
	XR_API extern range<writer::iterator> block_write(writer& wr, const range<writer::iterator>& buf);
	template<typename Ar>
	...
	XR_API extern long_size_t copy_data(reader& rd, writer& wr, long_size_t max_size = ~0);

	XR_API extern long_size_t copy_data(reader& rd, write_map& wr, long_size_t max_size = ~0);
	XR_API extern long_size_t copy_data(read_map& rd, writer& wr, long_size_t max_size = ~0);
	XR_API extern long_size_t copy_data(read_map& rd, write_map& wr, long_size_t max_size = ~0);
	...
	XR_EXCEPTION_TYPE(read_exception);
	XR_EXCEPTION_TYPE(write_exception);
	XR_EXCEPTION_TYPE(seek_exception);
    XR_EXCEPTION_TYPE(create_exception);


}}
#endif //end XR_COMMON_IARCHIVE_H__
