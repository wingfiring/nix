...
#ifndef XR_COMMON_MPL_H__
#define XR_COMMON_MPL_H__

#include <type_traits>
namespace xirang2{ namespace mpl{
...
struct all_of{
	static const bool value = F<T>::value && all_of<F, Args...>::value;
};
template<template<typename> class F, typename T>
struct all_of<F, T>{
	static const bool value = F<T>::value;
};
...
template<typename Seq> struct empty{ static const bool value = (size<Seq>::value == 0);};
...
#if 0		//FIXME:CLang can't support the specialization, need some investigation
