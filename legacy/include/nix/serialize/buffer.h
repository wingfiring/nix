#ifndef XR_COMMON_IO_BUFFER_H__
#define XR_COMMON_IO_BUFFER_H__
#include <xirang2/serialize/locals11n.h>
...
namespace xirang2{ namespace io{ namespace local{
	...
	Ar& save(Ar& ar, const buffer<T>& buf)
	{
		...
		if (!buf.empty())
		{
			...
			get_interface<xirang2::io::writer>(ar.get()).write(make_range(first, last));
		}
		return ar;
	}
	...
	Ar& load(Ar& ar, buffer<T>& buf)
	{
		...
		if (size > 0)
		{
			...
			get_interface<xirang2::io::reader>(ar.get()).read(make_range(first, last));
		}
		return ar;
	}
...
#endif //end XR_COMMON_IO_BUFFER_H__
