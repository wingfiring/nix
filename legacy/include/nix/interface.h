...
//
#ifndef XR_COMMON_INTERFACE_H__
#define XR_COMMON_INTERFACE_H__
...
namespace xirang2
{
	...
	{
		static_assert(xirang2::private_::valid_interface<Args...>::value, "interfaces must not be convertible between each other");
		...
		template<typename... OArgs>
			...
			: interface_holder<Args>(
					rhs.get_ptr<typename private_::get_interface_type<Args>::type>(), rhs.get_target())...
			, target_holder<void>(rhs.get_target())
#else
			: target_holder<void>(rhs.get_target())
			, interface_holder<Args>(
					rhs.template get_ptr<typename private_::get_interface_type<Args>::type>(), rhs.get_target())...
#endif
		{
		}
		...
		T* get_ptr() const {
			...
			const interface_holder<type>* self_ = this;
			return self_->get_interface();
		}
		template<typename T>
		typename xirang2::private_::get_return_type<T, Args...>::type
		get() const {
		...
		template<typename Ret, typename Interface>
			...
			const interface_holder<Interface>* self_ = this;
			return *self_->get_interface();
			}
		template<typename Ret, typename Interface>
		...
		struct iauto : public iref<Args...>
	{
        ...
        typedef iref<Args...> ref_t;
		iauto(){}
        
        ref_t& as_ref(){ return *this;}
        const ref_t& as_ref() const{ return *this;}
...
#endif //end XR_COMMON_INTERFACE_H__
