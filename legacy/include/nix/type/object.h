#ifndef XIRANG2_OBJECT_H__
#define XIRANG2_OBJECT_H__
...
namespace xirang2 { namespace type{
	...
	class XR_API ConstCommonObject
	{
		public:

            /// ctor
			explicit ConstCommonObject (Type t = Type(), const void *p = 0);
	...
	class XR_API CommonObject : public ConstCommonObject
	{
		public:
            /// ctor
			explicit CommonObject (Type t = Type(), void *p = 0);
	...
	class XR_API ConstSubObject
	{
		public:
			const static size_t PhonyIndex = size_t(-1);
			...
			explicit ConstSubObject (Type t = Type(), const void *p = 0, std::size_t idx = 0);
            ...
            /// get the member info of this item
            TypeItem itemInfo() const;
	...
	class XR_API SubObject : public ConstSubObject
	{
		public:
            /// ctor
            /// \pre (p == 0 && !t.valid() && idx == 0) || (p != 0 && t.valid()
			///		&& (idx < t.memberCount() || idx == PhonyIndex))
            /// \pre p == 0 || idx == PhonyIndex || t.member(idx).type().valid()
			explicit SubObject (Type t = Type(), void *p = 0, std::size_t idx = 0);
	...
	class XR_API ConstSubObjIterator
	{
		public:
            /// default ctor
			ConstSubObjIterator ();
	...
	class XR_API SubObjIterator : public ConstSubObjIterator
	{
		public:
            /// ctor
			SubObjIterator ();
    ...
    public:
        ...
        /// \post return.valid()
        CommonObject create(Type t, Namespace ns, const file_path& name);
        ...
        /// \post return.valid()
        CommonObject clone(ConstCommonObject obj, Namespace ns, const file_path& name);
    ...
    public:
        ...
        /// \throw nothrow
        void destroy(CommonObject obj,  Namespace ns, const file_path& name) const;

        /// destroy the given object and remove it from namespace
        /// \pre obj.valid() && *ns.findObject(name).second == obj
        /// \throw nothrow
        void operator()(CommonObject obj,  Namespace ns, const file_path& name) const;
    ...
    /// it's used to write exception safety code
	class XR_API ScopedObjectCreator
    {
    public:
        /// move construct
        ...
        CommonObject adoptBy(Namespace ns, const file_path& name);

        CommonObject adoptBy(Namespace ns, const file_path& name, const version_type& version);
	...
	XR_API extern type::CommonObject cloneObject(type::ConstCommonObject from, 
			heap & al = memory::get_global_heap(), ext_heap& eh = memory::get_global_ext_heap());
}}
#endif				//end XIRANG2_OBJECT_H__
