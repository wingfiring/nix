#ifndef XR_UNIQUE_PTR_H__
#define XR_UNIQUE_PTR_H__
  ...
  template<typename T>
    ...
    struct unify_delete<void>
    {
		...
		static void destroy_(void* ptr){
			XR_PRE_CONDITION(false && "just a stub, must not be called.");
    ...
    public:
        ...
        explicit unique_ptr(pointer p_)
            : ptr(p_), dp()
        {}
	...
	template<typename T, typename Dp>
        ...
        inline typename std::enable_if<!std::is_array<T>::value,
            unique_ptr<T> >::type make_unique(Others&&... args)
        {
            return (unique_ptr<T>(new T(std::forward<Others>(args)...)));
        }

        template<class T> inline
            typename std::enable_if<std::is_array<T>::value && std::extent<T>::value == 0,
            unique_ptr<T> >::type make_unique(std::size_t size)
        {
        ...
        template<class T,
        class... Others> typename std::enable_if<std::extent<T>::value != 0,
            void>::type make_unique(Others&&...) = delete;

        template<class T, class... Others>
        inline typename std::enable_if<!std::is_array<T>::value,
            std::unique_ptr<T> >::type make_std_unique(Others&&... args)
        {
            return (std::unique_ptr<T>(new T(std::forward<Others>(args)...)));
        }

        template<class T> inline
            typename std::enable_if<std::is_array<T>::value && std::extent<T>::value == 0,
            std::unique_ptr<T> >::type make_std_unique(std::size_t size)
        {
            typedef typename std::remove_extent<T>::type elem;
        ...
        template<class T,
            class... Others> typename std::enable_if<std::extent<T>::value != 0,
            void>::type make_std_unique(Others&&...) = delete;
}



#endif //end XR_UNIQUE_PTR_H__
