#ifndef XR_COMMON_STRING_ALGO_STRING_H__
#define XR_COMMON_STRING_ALGO_STRING_H__
...
namespace xirang2{ namespace str_algo {
    ...
    basic_string<CharT> replace(basic_range_string<const CharT> src, CharT oldv, CharT newv){
    ...
    template<typename ContT, typename ValueT>
    bool contains_element(const ContT& src, ValueT var){
        return find(src, var) != src.end();
    }

    struct icase_compare {
        XR_API bool operator()(char lhs, char rhs) const;
    };

    //template<typename ContT, typename ContU>
    //bool contains(const ContT& src, const ContU& pattern) {
    //    auto result = std::search(src.begin(), src.end(), pattern.begin(), pattern.end());
    //    return result != src.end();
    //}

    //template<typename ContT, typename ContU, typename BinaryPred>
    //bool contains(const ContT& src, const ContU& pattern, BinaryPred pred) {
    //    auto result = std::search(src.begin(), src.end(), pattern.begin(), pattern.end(), pred);
    //    return result != src.end();
    //}
}
...
using std::find;
using str_algo::rfind;
using str_algo::contains_element;
//using str_algo::contains;
}
#endif //end XR_COMMON_STRING_ALGO_STRING_H__
