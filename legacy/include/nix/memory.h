...
#ifndef XR_MEMORY_H__
#define XR_MEMORY_H__
...
namespace xirang2
{
	...
	template<typename T> T* malloc_obj(heap& h){
		...
		return h.malloc(sizeof(T), __alignof(T), 0);
#else
	...
	struct heap_creator
	{
		typedef xirang2::unique_ptr<T, heap_deletor<T> > pointer_type;
	...
	namespace memory{
		...
		XR_API extern heap& get_global_heap();

        XR_API extern ext_heap& get_global_ext_heap();

		/// set the global heap.
		XR_API extern void set_global_heap(heap& newHeap);
	...
	struct offset_range {
		long_offset_t begin() const { return m_begin;}
		long_offset_t end() const { return m_end;}

		/// the default begin and end are zero
		offset_range() : m_begin(0), m_end(0){}
		/// \pre b <= e
		explicit offset_range(long_offset_t b, long_offset_t e)
			: m_begin(b), m_end(e){}

		/// \return end - begin
		long_size_t size() const { return m_end - m_begin;}
		...
		bool in(const offset_range& rhs) const{
			return m_begin >= rhs.begin()
				&& m_end <= rhs.end();
		}
		bool contains(const offset_range& rhs) const{
			return rhs.in(*this);
		}
		bool cross_left(const offset_range& rhs) const{
			return m_begin < rhs.m_begin
				&& m_end > rhs.m_begin;
		}
		bool cross_right(const offset_range& rhs) const{
			return m_begin < rhs.m_end
				&& m_end > rhs.m_end;
		}
	...
	struct XR_INTERFACE ext_heap : public heap
	{
	public: //heap methods
	...
	public:
		...
		{
			XR_PRE_CONDITION(rhs.m_handle != 0);
		}
		...
		{
#ifdef MSVC_COMPILER_
			return reinterpret_cast<pointer>( get_heap().malloc(n * sizeof (value_type), __alignof(T), hint));
#else
	...
	public:
		...
		*/
		...
		pointer allocate(size_type size = 1, const void* hint = 0){
			XR_PRE_CONDITION(is_null());
			XR_PRE_CONDITION(size > 0);
	...
	*/
	...
	public:
		...
		*/
		...
		{
			XR_PRE_CONDITION(get() != 0 && index < size());
			return get()[index];
		}
