...
void VfsTester::test_mount(xirang2::vfs::IVfs& vfs)
{
...
void VfsTester::test_on_empty(xirang2::vfs::IVfs& vfs)
{
    xirang2::vfs::VfsNodeRange children = vfs.children(sub_file_path());
	...
	BOOST_CHECK(st.path == sub_file_path() && "state root of vfs");
	BOOST_CHECK(st.owner_fs == &vfs);

    st = vfs.state(sub_file_path(literal("not found")));
	BOOST_CHECK(st.state == fs::st_not_found);
	BOOST_CHECK(st.path == sub_file_path(literal("not found")));
	BOOST_CHECK(st.owner_fs == &vfs);
    ...
    BOOST_CHECK_THROW((vfs.create<io::reader, io::writer, io::random>(sub_file_path(literal("not found")), xirang2::io::of_open)), fs::not_found_exception);
...
void VfsTester::test_modification(xirang2::vfs::IVfs& vfs)
{
    ...
    auto file1 = vfs.create<io::reader, io::writer, io::random>(file_path("dir1/file1"), xirang2::io::of_create);
    BOOST_CHECK(vfs.state(file_path("dir1/file1")).path == file_path("dir1/file1"));
    BOOST_CHECK(vfs.state(file_path("dir1/file1")).size == 0);
    BOOST_CHECK(vfs.state(file_path("dir1/file1")).state == fs::st_regular);

	xirang2::io::random& prange = file1.get<io::random>();
	BOOST_CHECK(prange.offset() == 0);
	BOOST_CHECK(prange.size() == 0);

	xirang2::io::writer& pwriter = file1.get<io::writer>();
    ...
    auto file_dest = vfs.create<io::reader, io::random>(file_path("dest"), xirang2::io::of_open);
...
void VfsTester::test_readonly(xirang2::vfs::IVfs& vfs)
{
    BOOST_CHECK(vfs.createDir(file_path("dir3")) != fs::er_ok);
	BOOST_CHECK(vfs.createDir(file_path("dir3/dir31")) != fs::er_ok);

    BOOST_CHECK_THROW((vfs.create<io::reader, io::writer, io::random>(file_path("dir1/file1"), xirang2::io::of_create_or_open)), fs::permission_denied_exception);
	vfs.sync();
...
BOOST_AUTO_TEST_CASE(rootfs_case)
{
    ...
    vfs.create<io::writer>(file_path("dir1/file1"), xirang2::io::of_create);
	BOOST_CHECK(root.copy(file_path("/dir1/file1"), file_path("/dir2/dest")) == fs::er_ok);

	VfsState st = root.locate(file_path("/dir2/dest"));
	BOOST_CHECK(st.state == fs::st_regular);
	BOOST_CHECK(st.size == 0);
	BOOST_CHECK(st.owner_fs == &vfs2);
