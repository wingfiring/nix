...
namespace xirang2{ namespace zip{
	namespace {
		...
		void setDateTime(file_header& h)
		{
			...
			struct std::tm* ptm = gmtime(&rawtime);
	...
	public:
		...
		zip_package_imp(io::read_map& pkg) : package(&pkg){
			...
			if (pos == rend) //not found
				XR_THROW(data_corrupted_exception)("end central dir signature is not found");
			...
			if (zip_cd_offset == uint32_t(-1) 
				...
				if (pos.base() < address.begin() + K_sizeof_zip64_locator)
					XR_THROW(data_corrupted_exception)("Failed to locate Zip64 end of central directory locator");
				mrd.seek(pos.base() - address.begin() - K_sizeof_zip64_locator);

				if (io::load<uint32_t>(loader) != K_sig_zip64_end_of_cd_locator)
					XR_THROW(data_corrupted_exception)("Failed to read Zip64 end of central directory locator");
				...
				if (io::load<uint32_t>(z64_cd_loader) != K_sig_zip64_end_cd)
					XR_THROW(data_corrupted_exception)("Zip64 end of central directory signature error");
			...
			while(cdrd.readable())
			{
				...
				if (io::load<uint32_t>(cdin) != K_sig_central_file_header) 
					XR_THROW(data_corrupted_exception)("central file header signature error");
	...
	const file_header* append_(IoType& ar, const file_header& h_, file_type type, zip_package_writer_imp * m_imp){
		...
		XR_PRE_CONDITION(h.method == cm_deflate || h.method == cm_store);
		XR_PRE_CONDITION(h.name.str().size() < uint16_t(-1) );
		XR_PRE_CONDITION(h.comments.size() < uint16_t(-1) );
		...
		h.local_header_size_ = uint16_t(header_size);
