...
namespace xirang2{ namespace io{
	...
	buffer_rd_view buffer_in::view_rd(ext_heap::handle h) const {
		XR_PRE_CONDITION((size_t)h.begin() <= (size_t)h.end());
		XR_PRE_CONDITION((size_t)h.end() <= size());
	...
	{
		XR_PRE_CONDITION(in_size_t_range(offset));
	...
	{
		XR_PRE_CONDITION(in_size_t_range(size));
		unuse(size);
		return unknow_size;
	}
    ...
    mem_writer::mem_writer(mem_writer&& rhs)
        : buffer_out(m_data)
        , m_data(std::forward<buffer<byte>>(rhs.m_data)){
    }
