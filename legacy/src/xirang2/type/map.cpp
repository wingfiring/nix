...
namespace xirang2{ namespace type{
	struct Comp
	{
		...
		{
			XR_PRE_CONDITION(key.valid());
			XR_PRE_CONDITION(key.methods().extension() != 0);
			XR_PRE_CONDITION(key.methods().extension()->compare != 0);
	...
	public:
		...
		{
			XR_PRE_CONDITION(m_key == other.m_key);
			XR_PRE_CONDITION(m_value == other.m_value);
			clear();
		...
		{
			XR_PRE_CONDITION(key.type() == m_key);
			void* pk = clone_(key);
		...
		{
			XR_PRE_CONDITION(k.type() == m_key);
			XR_PRE_CONDITION(v.type() == m_value);
