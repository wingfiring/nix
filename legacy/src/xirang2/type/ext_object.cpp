#include <xirang2/type/ext_object.h>
...
namespace xirang2 { namespace type{
	...
	{
		XR_PRE_CONDITION(m_type == rhs.type());
	...
	{
		XR_PRE_CONDITION(m_counter == 0 && rhs.m_counter == 0);
	...
	{
		Type t = obj.type();
		XR_PRE_CONDITION(t.valid() && t.argCount() == 1);

		TypeArg ta = t.arg(0);
		XR_PRE_CONDITION(ta.name() == literal("value_type"));
