...
#include <aio/xirang2/vfs.h>
#include <aio/xirang2/object.h>
#include <aio/xirang2/vfs/local.h>
#include <aio/xirang2/vfs/zip.h>
...
public:
		...
		, m_xr("demo", aio::memory::get_global_heap(), get_ext_heap())
	{
		...
		m_global_dispatcher["xirang2"] = std::bind(&session::cmd_xirang, this, _1);
	...
	void cmd_xr_init(const std::string& txt){
		if (m_initialized)
		{
			std::cerr << "xirang2 is initialized\n";
			return;
		}
	...
	void cmd_xr_ls(const std::string& txt){
		...
		std::cout << "namespace:\n";
		std::for_each(sub_ns.begin(), sub_ns.end(), [](xirang2::Namespace ns){ std::cout << ns.name().c_str() << " ";});
		std::cout << "\ntypes:\n";
		std::for_each(types.begin(), types.end(), [](xirang2::Type t){ std::cout << t.name().c_str() << " ";});
		std::cout << "\nalias:\n";
		std::for_each(alias.begin(), alias.end(), [](xirang2::TypeAlias a){ std::cout << "[" << a.name().c_str() << "," << a.type().fullName().c_str() << "] ";});
		std::cout << "\nobjects:\n";
        std::for_each(objects.begin(), objects.end(), [](xirang2::NameValuePair obj){ std::cout << "[" << obj.name->c_str() 
	...
	void cmd_xr_lstype(const std::string& txt){
		...
		{
			std::cout << "type args:\n";
			std::for_each(args.begin(), args.end(), [](xirang2::TypeArg a){ std::cout << "[" << a.name().c_str() << "," << a.typeName().c_str();
					if (a.type().valid())
		...
		{
			std::cout << "bases:\n";
			std::for_each(bases.begin(), bases.end(), [](xirang2::TypeItem a){ std::cout << "[" << a.name().c_str() << " " 
					<< a.typeName().c_str();
					if (a.type().valid())
						std::cout << "(" << a.type().fullName() << ")";
		...
		{
			std::cout << "members:\n";
			std::for_each(members.begin(), members.end(), [](xirang2::TypeItem a){ std::cout << "[" << a.name().c_str() << " " << a.typeName().c_str();
					if (a.type().valid())
						std::cout << "(" << a.type().fullName() << ")";
					std::cout << " off=" << a.offset() << "] ";});
			std::cout << "\n";
	...
	{
		aio::string file = f;
		if (!xirang2::fs::is_absolute(file))
			file = m_pwd + file;

		auto state = m_vfs.locate(file);
		if (state.state == xirang2::fs::st_not_found)
		{
		...
		XR_PRE_CONDITION(state.node.owner_fs != 0);
	...
	void cmd_vfs_rm(const std::string& txt){
		...
		aio::string path = 
			xirang2::fs::is_absolute(pos->c_str())
			? pos->c_str()
			: xirang2::fs::normalize(m_pwd + pos->c_str());
		
		auto st = m_vfs.locate(path);

		if (st.state == xirang2::fs::st_not_found)
		{
			std::cerr << "file not found\n";
			return;
		}
	...
	void cmd_vfs_mount(const std::string& txt){
		...
		do {
			...
			if (!xirang2::fs::is_absolute(mpoint))
			{
			...
			if (sub_cmd == "local")
			{
				if (++pos != tok.end())
					break;
				std::shared_ptr<xirang2::fs::IVfs> pfs(new xirang2::fs::LocalFs(mpath));
				if (m_vfs.mount(mpoint, *pfs) == xirang2::fs::er_ok)
				{
			...
			if(sub_cmd == "zip")
			{
				...
				std::shared_ptr<xirang2::fs::IVfs> pfs(new xirang2::fs::LocalFs(cache));

				std::shared_ptr<xirang2::fs::IVfs> zipfs(new xirang2::fs::ZipFs(*par, mpath, *pfs));
				if (m_vfs.mount(mpoint, *zipfs) == xirang2::fs::er_ok)
				{
	...
	void cmd_vfs_unmount(const std::string& txt){
		...
		aio::string mpoint = xirang2::fs::append_tail_slash(pos->c_str());
	...
	const char* perror(xirang2::fs::FsError err)
	{
		const char* err_msg[] = {
			"ok",
			"invalid",
		...
		XR_PRE_CONDITION(err <= xirang2::fs::er_not_dir);
	...
	void cmd_vfs_cd(const std::string& txt){
		...
		else
		{
			...
			if (!xirang2::fs::is_absolute(new_dir))
			{
				aio::string p = m_pwd + new_dir;
				new_dir = xirang2::fs::append_tail_slash(xirang2::fs::normalize(p));
			}
			auto st = m_vfs.locate(new_dir);
			if (st.state == xirang2::fs::st_dir
					|| st.state == xirang2::fs::st_mount_point)
				m_pwd = new_dir;
			else
	...
	void cmd_vfs_cp(const std::string& txt){
		...
		aio::string path_src = 
			xirang2::fs::is_absolute(from.c_str())
			? from.c_str()
			: xirang2::fs::normalize(m_pwd + from.c_str());
		...
		aio::string path_dest = 
			xirang2::fs::is_absolute(dest.c_str())
			? dest.c_str()
			: xirang2::fs::normalize(m_pwd + dest.c_str());

		auto dst_st = m_vfs.locate(path_dest);

		if (dst_st.state == xirang2::fs::st_not_found 
				|| dst_st.state == xirang2::fs::st_regular)
		{
		...
		else if (st.state != xirang2::fs::st_dir)
		{
			return;
		}
	...
	void ls_file(const aio::string& path){
		...
		if (st.state == xirang2::fs::st_dir
				|| st.state == xirang2::fs::st_mount_point)
		{
			aio::string dir_parent =  st.node.path.empty() ? st.node.path : xirang2::fs::append_tail_slash(st.node.path);
			...
			for (auto itr = rng.begin(); itr !=rng.end(); ++itr)
			{
				...
				if (fst.state == xirang2::fs::st_dir
						|| fst.state == xirang2::fs::st_mount_point)
				{
					name += "/";
				}
...
private:
	...
	bool m_initialized;
	xirang2::fs::RootFs m_vfs;
	xirang2::Xirang m_xr;

	std::map<xirang2::fs::IVfs*, std::shared_ptr<xirang2::fs::IVfs> > m_fs_map; 
    std::map<xirang2::fs::IVfs*, std::shared_ptr<xirang2::fs::IVfs> > m_related_fs; 
    std::map<xirang2::fs::IVfs*, std::shared_ptr<aio::archive::iarchive> > m_related_archive; 
	aio::string m_pwd;
